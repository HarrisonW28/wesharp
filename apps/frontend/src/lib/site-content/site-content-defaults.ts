/** Mirrors `App\Support\SiteContent\SiteContentDefaults` — kept in sync for offline / API-failure fallback. */
export const SITE_CONTENT_DEFAULTS = {
  homepage: {
    hero_badge: "Greater Manchester & Liverpool",
    hero_title: "Professional knife sharpening, collected from your door.",
    hero_subtitle:
      "Book a slot, hand over your blades, and get them back sharp and inspected — without leaving your kitchen off the pass for long.",
    hero_supporting:
    ...
    how_steps: [
      {
        ...
        body: "Pick a date and time window that suits your kitchen — online, in a few minutes.",
      },
      {
        step: 2,
        title: "We collect your knives",
        body: "Our driver arrives, logs each blade, and takes them safely to our workshop.",
      },
      {
        step: 3,
        title: "We sharpen and inspect them",
        body: "Professional edges, a careful quality check, and safe handling throughout.",
      },
      {
        step: 4,
        title: "We return them ready to use",
        body: "Your knives come back sharp, ready for service — with clear updates in your account.",
      },
    ],
    ...
    benefits: [
      {
        ...
        body: "We come to you — no need to parcel knives or lose a whole service day.",
      },
      {
        title: "Workshop-sharp edges",
        body: "Proper equipment and experienced sharpeners on every job.",
      },
      {
        title: "Tracked orders & updates",
        body: "See where things stand in your WeSharp account from collection to return — including photos when your programme includes them.",
      },
      {
        title: "Straightforward £ pricing",
        body: "Clear quotes and GBP invoices so you always know what you're paying.",
      },
      {
        title: "Regular visits & care plans",
        body: "Ideal for busy kitchens — rolling routes and subscription-style programmes tailored to you.",
      },
    ],
  ...
  faq: [
    ...
    {
      q: "What if a knife is damaged in your care?",
      a: "We log condition at handover and again in the workshop. If anything doesn't look right, we pause and contact you before sharpening. Specific terms can be agreed when you're on-boarded as a regular customer — use Contact if you need paperwork in advance.",
    },
    {
      q: "Can you follow our RAMS or site rules?",
      a: "Yes. Share access rules, inductions, or RAMS when you book or via Contact — we'll work with your estates or H&S contact so collections fit your site policy.",
    },
    {
      q: "What does the business portal dashboard show?",
      a: "After sign-in you get an overview of your next collection, orders still in progress, and invoices that need attention — plus full lists for bookings, orders, knives, and invoices. On subscription programmes, allowance summaries appear on Your plan. Internal WeSharp analytics are separate; customer views are scoped to your organisation.",
    },
    {
      q: "Who gets logins on a trade account?",
      a: "During onboarding we agree billing contacts and site operators, then you invite users into the same tenant portal so everyone sees consistent status. Tell us if roles change and we can adjust access.",
    },
    {
      q: "Can groups get consolidated invoicing?",
      a: "Yes for typical trade setups — we align invoice recipients and references with your finance team. Mention multi-site on Contact or when you book so we quote and onboard correctly.",
    },
  ],
  faq_page: {
    title: "FAQ",
  ...
  booking: {
    ...
    page_lead:
      "Tell us about your kitchen and when you need us. We'll reply to confirm a slot and anything we need before we arrive — you don't need an account for this step.",
    success_kicker: "Enquiry received",
    success_title: "We'll be in touch soon",
    success_intro:
      "Our team will review your request and contact you using the email and phone you provided to confirm timing and any access details.",
    success_bullets: [
...
function mergeDeep(base: Record<string, unknown>, patch: unknown): Record<string, unknown> {
  if (!isRecord(patch)) {
    return base;
  }
  const out = { ...base };
  for (const [key, val] of Object.entries(patch)) {
    ...
    const b = out[key];
    if (Array.isArray(b) && Array.isArray(val)) {
      out[key] = val;
    } else if (isRecord(b) && isRecord(val)) {
      out[key] = mergeDeep(b, val);
    } else {
      out[key] = val;
    }
  }
...
/** Deep-merge API `content` into defaults (associative objects recurse; arrays replaced). */
export function mergeSiteContent(base: SiteContent, patch: unknown): SiteContent {
  return mergeDeep(base as unknown as Record<string, unknown>, patch) as SiteContent;
}
