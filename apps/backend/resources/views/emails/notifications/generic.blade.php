      ...
      <div style="background:#ffffff;border:1px solid #e6e7ea;border-radius:12px;padding:20px;">
        ...
        @if(!empty($ctaUrl) && !empty($ctaLabel))
          <p style="margin:0 0 8px 0;">
            <a href="{{ $ctaUrl }}" style="display:inline-block;background:#111827;color:#ffffff;text-decoration:none;padding:10px 14px;border-radius:10px;font-size:14px;">
              {{ $ctaLabel }}
            </a>
          </p>
          <p style="margin:0;font-size:12px;color:#6b7280;">
            If the button doesn’t work, copy and paste this link: {{ $ctaUrl }}
          </p>
        @endif
      </div>
