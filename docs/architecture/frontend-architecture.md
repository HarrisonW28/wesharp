...
| Navigation | `src/config/navigation.ts` — CRM (`companies.view`), **Analytics** (`analytics.view`), Orders (`orders.view`), Knives (`knives.view`), **Invoices** (`invoices.view`), **Payments** (`payments.view`), routes (`routes.view`), **webhook inbox** (`system.tools.view`, Sprint 15.3). |
